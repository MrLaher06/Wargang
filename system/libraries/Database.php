...
class Database_Core {
	...
	public function cache_query( $sql, $nomCache, $config )
	{		
		$tag = end( explode( '.', $config ) );
		$resultats = false;
		$nomCache = md5( $nomCache );

		if( is_null( $this->_cache->get( $nomCache ) ) && Kohana::config( 'cache.actif' ) )
		{	
			if( $requet = $this->db->query( $sql ) )
			{
				foreach( $requet as $val )
					$resultats[] = $val;
			}
			$this->_cache->set( $nomCache, $resultats, array( $tag ), Kohana::config( $config ) );
		}
		else
			$resultats = $this->_cache->get( $nomCache );
