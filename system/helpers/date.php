...
class date_Core {
	...
	public static function FormatDate ( $date, $format = '%d %B %Y - %H:%M', $offset = 0 )
	{		
		if( $date && preg_match( "/([0-9]{4})-([0-9]{2})-([0-9]{2})[ ]([0-9]{2}):([0-9]{2}):([0-9]{2})/", $date, $regs ) ) 
		{
			...
			return htmlentities( utf8_decode( $date ) );
		}
		else
			return false;	
	}
	...
	public static function Now_simple ( )
	{
		return date( 'Y-m-d' );
	}
	
	public static function convertir_date( $seconde = 0 )
	{
		if(!$seconde)
			$seconde = 0;
			
		if( $seconde < 60 )
			return date('s',$seconde) . ' s';
			
		elseif( $seconde < 3600 )
			return date('i',$seconde) . ' min';
			
		elseif($seconde < 86400)
			return date('H:i:s',$seconde);
		
		return date('d/m/Y à H:i:s',$seconde);
	}
	
	public static function unix2mysql($time = NULL) {
